...
def _axis_intervals(
    ...
    if n is None and groups is None and positions is None:
        raise ValueError('At least one of `n`, `groups` or `positions` must be defined')
    if n is None and groups is not None:
        n = len(groups)
    if n is None and positions is not None:
        n = len(positions)
...
def locus_intervals(
    ...
    step: Optional[int]=1, 
    target_chunk_size: Optional[int]=None,
    contigs: Optional[str]='variant_contig',
    positions: Optional[str]='variant_position',
    dtype: Union[str, Type]='int32',
) -> Tuple[DataArray, DataArray]:
    groups = ds[contigs]
    positions = ds[positions] if unit == 'physical' else None
    n = groups.shape[0]
